...
class gran_premioDAO:
    ...
    _SELECCIONAR_GRAN_PREMIO = 'SELECT Id_Gran_Premio,Nombre,Distancia,Num_Carreras WHERE Id_Gran_Premio=%s FROM Gran_Premio '
    ...
    @classmethod
    def seleccionar_un_gran_premio(cls,id):
        conexion = get_mysql_conection()
        with conexion:
            with conexion.cursor() as cursor:
                cursor.execute(cls._SELECCIONAR_GRAN_PREMIO,id)
                registros = cursor.fetchall()
                for registro in registros:
                    gran_pre = gran_premio(registro[0], registro[1], registro[2], registro[3])
...
if __name__ == "__main__":
    ...
    #print(gran_premioDAO.seleccionar_un_gran_premio(1))                                                                    #selecionamos la lista de apostantes de base de datos y mostramos el primero
